...
size_window = dataTable[2,2] - dataTable[1,2]    ##### NA ou non !!!!!   mean 2 by by

window_size_bench = dataTable[which(dataTable$chromosome == 1),2][1:500]
...
for (i in (1:(l-1))){
  listo = c(listo, window_size_bench[i+1] - window_size_bench[i])  
}

size_window = median(listo)
...
dataTable = dataTable[which(is.na(dataTable$ratio) != TRUE),]  
